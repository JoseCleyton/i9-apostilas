...
export class ListVideoLessonsComponent implements OnInit {
  ...
  public selectVideoLesson(_id: string) {
    ...
    this.formEditVideoLesson.get('description').setValue(this.videoLesson.description);
  ...
  public updateVideoLesson() {
    this.status = true;
    let videoLesson = new VideoLesson(this.formEditVideoLesson.value.title, this.formEditVideoLesson.value.link, this.videoLesson.clas, this.formEditVideoLesson.value.description);
    ...
    this.adminService.updateVideoLesson(videoLesson).subscribe(
      () => {
        ...
        this.status = false;
        this.adminService.getVideosLesson().subscribe((data: any) => this.videosLesson = data.videosLesson);
      },
      () => {
